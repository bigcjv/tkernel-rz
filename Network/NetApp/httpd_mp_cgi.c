/***************************************************************************
    ...
    Copyright (c)  2016-2017, eForce Co., Ltd. All rights reserved.
...
ER httpd_mp_content_rcv(T_HTTP_SERVER *http)
{
    ...
    if (rdsz <= http->rxlen) {
        ...
        rdsz -= http->rxlen;
        ercd = rcv_soc(http->SocketID, &http->rbuf[http->rxlen], (UH)(rdsz - 1U));
        if (0 < ercd) {
...
#define USER_NOTIFY_CBK(_evt_, _dat_, _len_, _ret_)                    \
    do { \
        hm.http = http;  hm.stat = (UB)(_evt_);  hm.dat = (VP)(_dat_);  hm.len = (UH)(_len_);  \
        *(_ret_) = notify_cbk(&hm);   \
    } while (0)
...
ER httpd_mp_cgi(T_HTTP_SERVER *http, const T_HTTP_FILE *fp)
{
    ...
    if (E_OK != ercd) {
        USER_NOTIFY_CBK(MPCGI_STAT_END, (VP)ercd, sizeof(ercd), &cbke);
        return ercd;
    }
